...
export interface NavMarket {
  ...
  /** Wall-clock ms of the last SUCCESSFUL read — lets buy tickets refuse
      quotes anchored to a stale price (mirrors the 45s options gate). */
...
export function useNavMarket(): NavMarket {
  const [m, setM] = useState<NavMarket>({ status: "loading", priceEth: null, updatedAt: null });
  useEffect(() => {
    let stop = false;
    const read = async () => {
      try {
        ...
        if (!stop) setM({ status: "ok", priceEth, updatedAt: Date.now() });
      } catch {
        /* keep last-good price (updatedAt keeps ageing so staleness is visible) */
        if (!stop) setM((prev) => (prev.status === "ok" ? prev : { status: "error", priceEth: null, updatedAt: null }));
      }
    };
    void read();
...
export interface VaultState {
  ...
  /** Vault registry assets with on-chain balances (via multicall).
      balance === null ⇔ that read failed — unknown, NEVER rendered as zero. */
  holdings: { address: Address; balance: bigint | null }[] | null;
...
export function useVaultState(): VaultState {
  ...
  useEffect(() => {
    ...
    /* Self-healing poll (was: one-shot fetch — a single transport blip on
       first paint left F3 dead until remount). On success: 60s steady-state
       refresh so AUM/holdings track the chain during a session. On failure:
       5s→10s→20s→40s backoff; last-good "live" state is KEPT (stale-while-
       revalidate) — only degrade to "error" when there has never been data.
       The redeem guard still fails closed: inactiveAssets stays null until a
       successful read proves the registry state. */
      ...
      try {
        ...
        setState({
          ...
          holdings: assets.map((a, i) => ({
            address: a,
            /* A-04: a failed balance read is UNKNOWN — never fabricate 0n. */
            balance: balances[i].status === "success" ? (balances[i].result as bigint) : null,
          })),
          inactiveAssets: inactive,
        });
        errDelay = 0;
        timer = setTimeout(read, 60_000);
      } catch {
        if (stop) return;
        setState((s) => (s.status === "live" ? s : { ...s, status: "error" }));
        errDelay = Math.min(errDelay ? errDelay * 2 : 5_000, 40_000);
        timer = setTimeout(read, errDelay);
      }
    };
    void read();
