...
export async function connectWallet(rdns?: string): Promise<void> {
  ...
  try {
    ...
    adopt(pick, acc, cid);
    // M-11: wallets with fixed network lists (Phantom in-app browser) connect
    // on their default EVM chain (Ethereum). Prompt the switch immediately so
    // the session is never parked on "wrong-chain" without an affordance.
    // Failures are non-fatal: the session stays connected on the wrong chain
    // and the picker/topbar surface the switch CTA.
    if (cid !== robinhoodChain.id) {
      try {
        if (await ensureChain()) {
          // Belt-and-braces: some providers do not emit chainChanged after a
          // programmatic switch — re-read and reconcile.
          const cid2 = asChainId(await withTimeout(pick.provider.request({ method: "eth_chainId" })));
          if (cid2 !== null && state.account) {
            setState({ chainId: cid2, status: cid2 === robinhoodChain.id ? "connected" : "wrong-chain" });
          }
        }
      } catch {
        /* connected on the wrong chain — UI shows the switch CTA */
      }
    }
  } catch {
...
export async function ensureChain(): Promise<boolean> {
  ...
  /* Already on Robinhood Chain — never re-prompt (strict wallets double-prompt). */
  if (state.chainId === robinhoodChain.id) return true;
