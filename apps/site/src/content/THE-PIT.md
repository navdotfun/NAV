...
> **Status: LIVE on Robinhood Chain mainnet.** All 22 Pit contracts are deployed and source-verified on Blockscout — 18 PitPool markets plus factory, oracle, ticket and deployer infrastructure (addresses below).
...
The Pit's contracts passed the full NAV security programme prior to deployment:
...
6. **Remediation on the record** — every defect surfaced by the campaign was fixed and re-verified before deployment; sanitized findings are published in the official repository.
7. **Third-party audit** is planned before unrestricted mainnet exposure, alongside the core protocol audit.

## Deployed addresses — Robinhood Chain mainnet (LIVE, verified)

All contracts source-verified on Blockscout (solc 0.8.36). Superseded early deployments were retired with zero user funds and zero open interest, and remain verified on-chain for provenance.
...
Strike spacing ≈2.5% of spot at configuration, snapped to 1/2/5 steps; σ (annualized, immutable per pool) set per asset class. Quote asset USDG. New markets are owner-gated: `PitFactory.createPool` (onlyOwner) accepts any vault-listed asset once its oracle market is configured — the 18 below are the launch set, not a cap.
...
**Delisted markets.** RDDT, COST and NFLX pools were retired when on-chain verification established that no Chainlink Data Feed exists on Robinhood Chain for those tickers; QQQ, META and AMD replaced them with full Chainlink + Pyth + TWAP oracle coverage. An MSTR pool is deployed but not offered while its AMM pool trades at a material dislocation from the Chainlink anchor — the oracle fails closed on quoting until the dislocation normalises. All retired contracts remain deployed, verified and solvent with zero open interest.
