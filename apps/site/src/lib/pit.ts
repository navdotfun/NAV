...
export async function fetchPositions(account: Address): Promise<PitPosition[]> {
  ...
  ids.forEach((id, i) => {
    const r = owners[i];
    /* A failed ownerOf means "burned" ONLY when we scanned the full id space.
       Under log-based discovery every id here provably existed, so a failure is
       an RPC error and must not be cached as a permanent burn. */
    if (r.status !== "success") { if (!fromLogs) burnedIds.add(id.toString()); return; }
  ...
  if (uniqueUnderlyings.length > 0) {
    const mults = await publicClient.multicall({
