...
const navCrankAbi = [
  ...
  { type: "function", name: "rotationList", stateMutability: "view", inputs: [], outputs: [{ type: "address[]" }] },
  {
    type: "event", name: "Cranked", inputs: [
      ...
      { name: "bought", type: "uint8", indexed: false },
...
export async function readNavCrankState(): Promise<NavCrankState> {
  const [lastCrankAt, cooldown] = await Promise.all([
    publicClient.readContract({ address: PROTOCOL.navCrankAddress!, abi: navCrankAbi, functionName: "lastCrankAt" }),
    publicClient.readContract({ address: PROTOCOL.navCrankAddress!, abi: navCrankAbi, functionName: "crankCooldown" }),
  ]);
...
export async function readCrankFeed(): Promise<{ events: CrankEvent[]; leaders: CrankLeader[] }> {
  ...
  const logs: Awaited<ReturnType<typeof publicClient.getContractEvents<typeof navCrankAbi, "Cranked">>> = [];
  let hi = latest;
  for (let i = 0; i < FEED_MAX_CHUNKS && hi >= NAVCRANK_DEPLOY_BLOCK; i++) {
    const lo = hi - FEED_CHUNK + 1n > NAVCRANK_DEPLOY_BLOCK ? hi - FEED_CHUNK + 1n : NAVCRANK_DEPLOY_BLOCK;
  ...
  // one timestamp fetch per unique block (cranks are ≤48/day — this is tiny)
  ...
  const events: CrankEvent[] = logs
    .map((l) => ({
      caller: l.args.caller!, wethIn: l.args.wethIn!, navBurned: l.args.navBurned!,
      usdgOut: l.args.usdgOut!, bought: Number(l.args.bought!), reward: l.args.reward!,
      blockNumber: l.blockNumber, timestamp: stamps.get(l.blockNumber) ?? 0, txHash: l.transactionHash,
    }))
