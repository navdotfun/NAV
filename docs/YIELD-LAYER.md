...
The Pit is fully collateralized by design: every call is backed by escrowed stock tokens, every put by escrowed USDG. Full collateralization is what makes the Pit safe — but it also means capital sits idle between expiries. The yield layer fixes that without touching the Pit's safety model: writers can opt in to route idle collateral into concentrated Uniswap v4 liquidity bands, earning swap fees on top of option premiums. STORMM pioneered this shape; ours is adapted to weekly European settlement and the NAV fee flywheel.
...
**Pricing needs no keepers at all.** PitOracleV2 anchors every read to the market's Chainlink feed, falls back to Pyth automatically, and deviation-checks the pool TWAP against both — there are no heartbeats, pokes or schedules in the pricing path. (The earlier PokeBounty heartbeat contract is retired.)
